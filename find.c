...
I oflen(I l,S ls,M rs) { PF("length %d:\n",l); I pl=strlen(ls); S s=calloc(1,l+pl+1); strcpy(s,ls); M p,r; A a=nA(); I c, f=0;
	DO(ipow(8,l), prog(i,pl); SRED(pl); if (i%1000001==0) puts(s);
		if ((p=ps(s,0,a))&&(r=ex(p,&c,a),c>=0)&&teq(rs,r,0)&&PF("%s |-> ",s)&&pr(r,0)&&puts("")) { f=1; break; }; rc(a));
	fr(a); free(s); R f; }
...
I main(I argc,S argv[]) { P(argc!=3,puts(help)); S ls=argv[1]; A a=nA(); M rs=ps(ls,0,a); P(!rs, fr(a), puts("doesn't parse"));
	rs=ps(argv[2],0,a); P(!rs, fr(a), puts("doesn't parse")); DO(99,P(oflen(i,ls,rs), fr(a), 0)); fr(a); }
