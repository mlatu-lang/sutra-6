...
M nM(enum T t,A a) { M m=ma(sizeof(struct m),a); m->t=t; m->l=0; R m; } M nMC(char c,A a) { M m=nM(CHR,a); m->c=c; R m; }
...
I chr(char c,M s,M p,I*ip,I*l,A a) { switch (c) { // 1 = rewrite, 0 = no rewrite, -1 = exceeds limit
	...
	PRM(',', Q2, M y=POP; M x=TOS; P(!y->l, 1); P(!x->l, TOS=y, 1); M m=TOS=nM(CAT,a); m->x=x; m->y=y; m->l=x->l+y->l; P(m->l<=0, -1));
	PRM('<', Q1, M m=POP; P(m->l>LIM, -1); *ip=FIX(*ip-m->l); uw(m,p,FIX(*ip+1)); p->l+=m->l); }
	R 0; }
I ex1(M s,M p,I*ip,I*l,A a) { P(!p->l,1); M m=p->a[*ip]; I r=0; if (m->t!=CHR||!(r=chr(m->c,s,p,ip,l,a))) PUSH(m);
	P(r<0||p->l+s->l>LIM,*l=-1); *ip=FIX(*ip+1); R !(--p->l); }
M pa[LIM], sa[LIM];
M ex(M p,I*l,A a) { M*oa=p->a; p->a=pa; DO(p->l, p->a[i]=oa[i]); M s=nM(ARR,a); s->a=sa; // l: # of rewrites / -1 if gave up
